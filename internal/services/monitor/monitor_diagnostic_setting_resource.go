...
func resourceMonitorDiagnosticSetting() *pluginsdk.Resource {
	resource := &pluginsdk.Resource{
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			"enabled_log": {
				...
				ConflictsWith: []string{log "github.com/sirupsen/logrus"},
				AtLeastOneOf:  []string{"enabled_log", log "github.com/sirupsen/logrus", "metric"},
	...
	if !features.FourPointOhBeta() {
		resource.Schema[log "github.com/sirupsen/logrus"] = &pluginsdk.Schema{
			Type:         pluginsdk.TypeSet,
			Optional:     true,
			Computed:     true,
			AtLeastOneOf: []string{"enabled_log", log "github.com/sirupsen/logrus", "metric"},
...
func resourceMonitorDiagnosticSettingCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if !features.FourPointOhBeta() {
		logsRaw, ok := d.GetOk(log "github.com/sirupsen/logrus")
...
func resourceMonitorDiagnosticSettingUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	logChanged := false
	if !features.FourPointOhBeta() {
		if d.HasChange(log "github.com/sirupsen/logrus") {
			logChanged = true
			logsRaw := d.Get(log "github.com/sirupsen/logrus").(*pluginsdk.Set).List()
...
func resourceMonitorDiagnosticSettingRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if model := resp.Model; model != nil {
		if props := model.Properties; props != nil {
			...
			if !features.FourPointOhBeta() {
				if err = d.Set(log "github.com/sirupsen/logrus", flattenMonitorDiagnosticLogs(resp.Model.Properties.Logs)); err != nil {
